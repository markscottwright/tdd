map <leader>tt :wall<cr>:Shell manage.py test lists<cr>
map <leader>tf :wall<cr>:Shell manage.py test functional_tests<cr>
"map <leader>tf :wall<cr>:Shell manage.py test functional_tests.test_list_item_validation<cr>
"set mp=pep8\ --ignore=E266\ d:\\workspaces\\tdd\\superlists
set mp=flake8\ --ignore=E265\ --exclude=migrations\ d:\\workspaces\\tdd\\superlists
set tw=79
